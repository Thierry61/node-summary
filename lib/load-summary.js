...
function add_to_hash_map(hash, time, difficulty, height, current_height) {
    // Remove oldest entry but not the first of current diff epoch, nor the last of previous diff epoch and nor current height.
    // Note that entries are in the order of entry insertion.
    if (hash_map.size >= hashMapCapacity) {
        const diff_epoch_first_block_height = Math.trunc(current_height / difficultyAdjustmentBlockCount)
        let old_key = undefined
        for (const [key, value] of hash_map) {
            if (value.height != diff_epoch_first_block_height && value.height != diff_epoch_first_block_height - 1 && value.height != current_height) {
                old_key = key;
                break;
            }
        }
...
async function loadSummaryCached() {
    try {
        ...
        for (let i = 0; i < rpcResults.length; i++) {
            ...
            switch (method) {
                case "getblock":
                    add_to_hash_map(res.bestblockhash, rpcResult.time, rpcResult.difficulty, rpcResult.height, res.headers)
                    break
                case "getblockhash":
        ...
        if (methods.length > 0) {
            ...
            for (let i = 0; i < rpcResults.length; i++) {
                ...
                switch (method) {
                    case "getblock":
                        add_to_hash_map(rpcResult.hash, rpcResult.time, rpcResult.difficulty, rpcResult.height, res.headers)
                        break
                    default:
                        // To analyze results of a new method just uncomment following line
                        // console.debug(rpcResult)
