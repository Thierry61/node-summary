    ...
    try {
        ...
        if (!isUndefinedOrEmpty("BITCOIND_COOKIE_PATH")) {
            ...
            if (isUndefinedOrEmpty("BITCOIND_USERNAME") || isUndefinedOrEmpty("BITCOIND_PASSWORD")) {
                throw("BITCOIND_COOKIE_PATH or BITCOIND_USERNAME/BITCOIND_PASSWORD environment variables are not defined")
            }
            userPass=`${process.env.BITCOIND_USERNAME}:${process.env.BITCOIND_PASSWORD}`
        }
...
function getLastRes() {
    ...
    if (cachedPromise == undefined) {
        cachedPromise = new Promise(async function (resolve, _) {
            const data = await loadSummaryCached()
            resolve(data)
        });
    }
...
export async function loadSummary() {
    ...
    if (!hit) {
        ...
        if (res.err == undefined) {
            const lastHeaders = lastRes.headers
            ...
            // Don't do it after Next js server reboot, or when previous request was at a different block height
            if (lastMempoolNtx != undefined && lastHeaders == res.headers) {
                // Recompute diffTime from new result
                const diffTime = (res.server_time - lastTime) / 1000
                const ntxPerSeconds = (res.mempool.ntx - lastMempoolNtx) / diffTime
                res.mempool.ntx_per_second = precision3(ntxPerSeconds)
            }
        }
        lastRes = res
    }
