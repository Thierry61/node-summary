        ...
        for (let i = 0; i < rpcResults.length; i++) {
            ...
            switch (method) {
                ...
                case "estimatesmartfee":
                    ...
                    // Correct cache when fees are lower for a shorter duration, or higher for a longer duration
                    // to work around possible inconsistencies when previous request was done a long time ago
                    const max_fee_index = Math.min(cached_fees.length, fees_durations.length)
                    for (let i = 0; i < fee_index; i++) {
                        cached_fees [i] = Math.max(cached_fees [i], feerate)
                    }
                    for (let i = fee_index + 1; i < max_fee_index; i++) {
                        cached_fees [i] = Math.min(cached_fees [i], feerate)
                    }
                    // Returns the cache
                    for (let i = 0; i < max_fee_index; i++) {
